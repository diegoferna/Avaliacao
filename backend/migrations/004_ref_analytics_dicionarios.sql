...
-- Status = valores usados em unidades.status e equipes.status.
...
-- ---------------------------------------------------------------------------
-- Status de unidades e equipes (código em unidades.status / equipes.status)
-- ---------------------------------------------------------------------------
...
COMMENT ON TABLE ref_status_entidade IS
    'Documentação dos valores de status; unidades e equipes só aparecem no app se status = ativo.';
...
-- Opcional: status coerente com dicionário (não valida valores legados fora da lista)
DO $$
BEGIN
...
COMMENT ON VIEW v_avaliacoes_analytics IS
    'Avaliações com rótulos da escala e pontos 0–10 por dimensão; use para BI/exportação.';
